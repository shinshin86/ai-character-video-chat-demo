...
停止するとコメント取得と待機列を停止・消去します。生成中・再生中の返答は完了まで続きます。API・生成・再生エラー時は自動返答を停止し、問題解決後に手動で再接続します。再読み込みや設定保存でも取得は停止します。自動返答ごとに通常のfal生成料金が発生し、コメント取得はYouTube APIの利用枠を消費します。動画生成に時間がかかる場合は、返答前にコメントが期限切れになることがあります。
